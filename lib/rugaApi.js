...
const wiki = async (url) => new Promise((resolve, reject) => {
    axios.get(`${link}/api/wikien?q=${url}`)
    .then((res) => {
...
const covid = async () => new Promise((resolve, reject) => {
	axios.get(`${link}/api/corona?country=india`)
	.then((res) => {
		const textv = `Info Covid-19 ${res.data.country}\n\nToday cases: ${res.data.todaycases}\nTotal cases: ${res.data.total_cases}\nRecovered: ${res.data.recovered}\nActive: ${res.data.active}\ntotal Deaths: ${res.data.totaldeath}\n`
		resolve(textv)
	})
	.catch((err) => {
		reject(err)
